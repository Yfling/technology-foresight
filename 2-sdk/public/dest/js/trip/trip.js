(function(){
    ...
    var $list = $("#list");
        ...
        $.ajax({
            url: '/pull/tripList?type='+type+'&sort='+sort+'&keyword='+keyword + '&zone=' + zone
        }).done(function(msg){
    ...
    function render(data){
        ...
        for(var i=0,l=data.length;i<l;i++){
            if(data[i].status == '已完成'){
                ...
                var $sample = $("<a href='/tripDetail?id=" + data[i].id + "' class='sample'></a>");
                var $opcheck = $("<div class='opcheck'></div>");
                var $apply = $("<div id='"+data[i].id+"' class='apply'></div>");
                var $endcheck = $("<div class='endcheck'>"+
                        "<div class='title'>"+
